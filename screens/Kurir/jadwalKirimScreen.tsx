...
type PresetRange = 'month' | 'next7' | 'next14';
...
export default function KurirJadwalKirimScreen({ navigation }: any) {
  const { token } = useAuth();
  ...
  const applyPreset = useCallback((value: PresetRange) => {
    const now = new Date();
    if (value === 'month') {
      setTanggalAwal(dateToYmd(new Date(now.getFullYear(), now.getMonth(), 1)));
      setTanggalAkhir(
        dateToYmd(new Date(now.getFullYear(), now.getMonth() + 1, 0)),
      );
    }

    if (value === 'next7') {
      setTanggalAwal(dateToYmd(now));
      setTanggalAkhir(
        dateToYmd(
      ...
      try {
        ...
        Toast.show({
          ...
          text2:
            ...
            'Tidak dapat mengambil jadwal kirim',
        });
        setData([]);
      } finally {
        setLoading(false);
  ...
  const renderItem = useCallback(
    ({ item }: { item: JadwalItem }) => {
      const isDone = String(item.realisasi || 'N').toUpperCase() === 'Y';
      const tanggalPlan = normalizeYmd(String(item.tanggal_plan || ''));
          ...
          onPress={() => {
            if (isDone) {
              Toast.show({
                type: 'glassInfo',
                text1: 'Info',
                text2: 'Pengiriman ini sudah direalisasi',
              });
              return;
            }
  ...
  const ListHeader = (
    <View style={styles.headerWrap}>
      <View style={styles.header}>
        <Text style={styles.title}>Jadwal Kirim</Text>
        <Text style={styles.subtitle}>
          Rekap rencana kirim dan realisasi cepat
        </Text>
      </View>
      ...
      <View style={styles.chipRow}>
        {[
          ...
          { key: 'next7', label: '7 Hari' },
          { key: 'next14', label: '14 Hari' },
        ].map(opt => {
          ...
          return (
            <TouchableOpacity
              key={opt.key}
              activeOpacity={0.9}
              onPress={() => applyPreset(opt.key as PresetRange)}
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <Text style={styles.empty}>
              Belum ada jadwal kirim pada periode ini
            </Text>
          )
        }
