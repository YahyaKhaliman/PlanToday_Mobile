...
export default function KurirKirimScreen() {
  ...
  const [data, setData] = useState<KirimItem[]>([]);
  ...
  const fetchData = useCallback(
    async (nextPage = 1, silent = false) => {
      ...
      try {
        const res = await api.get('/kurir/kirim', {
          params: {
            page: nextPage,
            limit: 15,
        ...
        Toast.show({
          ...
          text2:
            ...
            'Tidak dapat mengambil rekap kiriman',
        });
        setData([]);
      } finally {
        setLoading(false);
  ...
  const renderItem = useCallback(({ item }: { item: KirimItem }) => {
    const tanggalItem = normalizeYmd(
      String(item.tanggal || item.tanggal_plan || ''),
    );
    return (
      <View style={styles.cardCompact}>
        ...
        <View style={[styles.compactBadge, styles.badgeDone]}>
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <Text style={styles.empty}>
              Belum ada kiriman selesai untuk periode ini
            </Text>
          )
        }
